...
def upload():
    ...
    if not MOCK_MODE:  # pragma: no cover
        ...
        # If this is an idempotent retry the track already has a real gcs_path — skip re-upload.
        existing_gcs = track.get('gcs_path', '')
        if existing_gcs in ('pending', '', None):
            ...
            gcs_path = existing_gcs
            # Derive public URL from gs:// path: gs://bucket/path → https://storage.googleapis.com/bucket/path
            audio_url = existing_gcs.replace('gs://', 'https://storage.googleapis.com/', 1)
